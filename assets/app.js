// app.js – Tiny brain for ProfitPlate v2
...
(function () {
      ...
      .map((p) => {
        ...
        hist = hist
          ...
          .map((h) => {
            ...
            const d = h.date || n.updatedAt || n.createdAt || new Date().toISOString();
            return {
              date: d,
        ...
        if (!hist.length && n.pricePerUnit > 0) {
          const baseDate = n.updatedAt || n.createdAt || new Date().toISOString();
          hist.push({ date: baseDate, pricePerUnit: n.pricePerUnit });
        }
  ...
  // ===========================
  // Guided tour demo loader
  // ===========================
  /**
   * Load sample data for the guided tour. If there are no existing purchases or
   * recipes, seed the same demo data used on first run. This enables users to
   * see how Purchases and Recipes pages look without adding everything by hand.
   */
  window.ppLoadSampleDataFromTour = function () {
    if (purchases.length > 0 || recipes.length > 0) {
      alert('You already have data. Remove it before loading the demo.');
      return;
    }
    const nowIso = new Date().toISOString();
    const demoPurchases = [
    ...
    const demoRecipes = [
      {
        ...
        isDemo: true,
        ingredients: [
          { purchaseId: 'p_demo_2', quantity: 0.5, unit: 'kg' },
          { purchaseId: 'p_demo_3', quantity: 0.2, unit: 'kg' },
          { purchaseId: 'p_demo_4', quantity: 5, unit: 'pcs' },
          { purchaseId: 'p_demo_1', quantity: 0.05, unit: 'kg' },
        ],
      },
    ];
    purchases = demoPurchases;
    ...
    safeSave(KEYS.purchases, purchases);
    safeSave(KEYS.recipes, recipes);
    alert('Demo data loaded. Check the Purchases and Recipes pages.');
  };
  // ===========================
  ...
  function initPurchasesPage() {
    ...
    function fillPriceFromPack() {
      ...
      const packQty = Number(String(packQtyInput.value || "").replace(",", ".")) || 0;
      const packUnit = packUnitSelect.value;
      const packPrice = Number(String(packPriceInput.value || "").replace(",", ".")) || 0;
      if (!baseUnit || !packUnit || !packQty || !packPrice) {
        alert("Fill pack quantity, pack unit and pack price before using the helper.");
        return;
      }
    ...
    function renderRecent() {
      ...
      latest.forEach((item) => {
        ...
        const priceLabel = price > 0 ? `${price.toFixed(4)} €/ ${item.unit || ""}` : "-";
        ...
        const supPart = item.supplier ? ` • Supplier: ${item.supplier}` : "";
        meta.textContent = `Updated ${formatDate(item.updatedAt)} • ${catPart}${subPart}${supPart}`;
        if (item.isDemo) {
    ...
    function renderPriceHistory(item) {
      ...
      sorted.forEach((entry) => {
        ...
        tdPrice.textContent = val.toFixed(4) + " €/ " + (item.unit || "");
    ...
    function renderTable() {
      ...
      purchases
        ...
        .filter((p) => {
          ...
          if (!q) return true;
          const haystack = [p.name || "", p.category || "", p.subtype || "", p.supplier || "", p.invoiceNumber || ""].join(" ").toLowerCase();
    ...
    purchaseForm.addEventListener("submit", function (e) {
      ...
      const price = Number(String(priceInput.value || "").replace(",", ".")) || 0;
