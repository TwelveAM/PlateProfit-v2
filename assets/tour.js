// assets/tour.js (v2)
// Improved guided tour with arrow pointing to highlighted element.

(function () {
  const TOUR_STATE_KEY = "pp2_tour_state_v1";
  ...
  const STEPS = [
    // HOME
    {
      ...
      text:
        "This is a static prototype that shows how you’ll track real dish cost and margin.\n" +
    ...
    },
    {
      ...
      text:
        "Use these tabs to move between Purchases (ingredient costs), Recipes (dishes), " +
        "and Settings (currency & behaviour).",
      panelPlacement: "bottom"
    },
    // PURCHASES
    {
      ...
      text:
        ...
        "any recipes using that ingredient will update automatically.",
      panelPlacement: "bottom"
    },
    // RECIPES
    {
    ...
    // SETTINGS – Behaviour
    {
      ...
      text:
        "These toggles control whether recipes auto-recalculate when ingredient prices change, " +
        "and whether to show advanced cost breakdowns by default.",
      panelPlacement: "bottom",
      highlightOffset: { top: 8, bottom: 8, left: 0, right: 0 }
    },
    // SETTINGS – Data & local storage
    {
      id: "settings-data",
      page: "settings",
      ...
      text:
        "For now everything lives only in your browser — no server, no cloud. " +
        "In a future version this is where imports, exports and backups will live.",
      panelPlacement: "bottom",
      showDemoButton: true,
      highlightOffset: { top: 4, bottom: 4, left: 0, right: 0 }
    }
  ];
  ...
  function renderCurrentStep() {
    ...
    if (targetRect) {
      const padding = 12;
      const offset = step.highlightOffset || {
        top: 0,
        right: 0,
        bottom: 0,
        left: 0
      };
      const top = targetRect.top - padding + (offset.top || 0);
      const left = targetRect.left - padding + (offset.left || 0);
      const width = targetRect.width + padding * 2 - (offset.left || 0) - (offset.right || 0);
      const height = targetRect.height + padding * 2 - (offset.top || 0) - (offset.bottom || 0);
    ...
    const showDemo = !!step.showDemoButton && typeof window.ppLoadSampleDataFromTour === "function";
    panelEl.innerHTML = `
      ...
      <div class="pp-tour-footer">
        ...
        ${!isFirst ? '<button class="pp-tour-btn pp-tour-btn-secondary" data-pp-tour="back">Back</button>' : ''}
        ${showDemo ? '<button class="pp-tour-btn pp-tour-btn-secondary" data-pp-tour="demo">Load demo data</button>' : ''}
        <button class="pp-tour-btn pp-tour-btn-primary" data-pp-tour="next">${isLast ? "Finish" : "Next"}</button>
      </div>
    `;
    // Position panel relative to target (or bottom-center if no target)
    setTimeout(() => {
      ...
      if (targetRect) {
        ...
        // Compute arrow offset relative to panel
        const arrowLeft = centerX - left;
        panelEl.style.setProperty('--arrow-left', `${arrowLeft}px`);
      } else {
        top = window.innerHeight - panelRect.height - 32;
        left = (window.innerWidth - panelRect.width) / 2;
        panelEl.style.setProperty('--arrow-left', `${panelRect.width / 2}px`);
      }
  ...
  function maybeResumeTourFromState() {
    ...
    if (!step || step.page !== PAGE_ID) {
      // State belongs to different page; do nothing here
      return;
    }
    currentIndex = idx;
    renderCurrentStep();
