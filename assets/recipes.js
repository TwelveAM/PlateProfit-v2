...
(function () {
  ...
  const settings = typeof window.loadSettings === "function" ? window.loadSettings() : {
    currency: "EUR",
    locale: "eu",
    autoRecalc: true,
    showAdvanced: true,
  ...
  function readFormToRecipe() {
    ...
    const ingRows = Array.from(ingredientsContainer.querySelectorAll(".recipe-ingredient-row"));
    const ingredients = [];
  ...
  function renderRecipes() {
    ...
    if (search) {
      visible = visible.filter((r) => (r.name || "").toLowerCase().includes(search));
    }
    ...
    visible.forEach((recipe) => {
      ...
      badge.textContent = formatMoney(costs.costPerPortion, 2) + " /portion";
      ...
      let summaryText;
      if (showAdvanced) {
        summaryText = `Total batch cost: ${formatMoney(costs.batchCost, 2)}` +
          ` • Cost per portion: ${formatMoney(costs.costPerPortion, 2)}`;
        if (recipe.sellingPrice != null) {
          summaryText += ` • Selling price: ${formatMoney(recipe.sellingPrice, 2)} /portion`;
          if (costs.marginPerPortion != null) {
            summaryText += ` • Margin: ${formatMoney(costs.marginPerPortion, 2)} (${(costs.marginPercent || 0).toFixed(0)}%)`;
          }
        }
      } else {
        summaryText = `Cost per portion: ${formatMoney(costs.costPerPortion, 2)}`;
        if (recipe.sellingPrice != null) {
          summaryText += ` • Selling price: ${formatMoney(recipe.sellingPrice, 2)} /portion`;
        }
      }
      ...
      if (showAdvanced) {
        ...
        thead.innerHTML = `<tr><th>Ingredient</th><th>Category</th><th>Sub-type</th><th>Supplier</th><th>Quantity</th><th>Unit</th><th>Price/unit</th><th>Cost in recipe</th></tr>`;
        table.appendChild(thead);
        const tbody = document.createElement("tbody");
        costs.ingredientsDetailed.forEach((ing) => {
          const tr = document.createElement("tr");
          tr.innerHTML = `<td>${ing.ingredientName}</td><td>${ing.category}</td><td>${ing.subtype}</td><td>${ing.supplier}</td><td>${ing.quantity}</td><td>${ing.unit}</td><td>${formatMoney(ing.pricePerUnit, 4)}/unit</td><td>${formatMoney(ing.costInRecipe, 2)}</td>`;
